...
### Recompra
Clientes promotores (nota 9–10) têm propensão significativamente maior de voltar a comprar. Em e-commerce, onde o custo de aquisição de novos clientes (CAC) é alto, reter um cliente satisfeito é muito mais rentável do que conquistar um novo. Um NPS baixo é um sinal antecipado de churn.
...
Para fins de modelagem preditiva, o `nps_score` foi convertido em 3 categorias seguindo a metodologia padrão do NPS:
...
> ⚠️ **Data Leakage com `csat_internal_score`:** Essa variável é um score interno de satisfação provavelmente calculado a partir do próprio NPS ou coletado no mesmo momento. Usá-la como feature do modelo seria uma forma de "trapacear" — o modelo estaria usando informação que não estaria disponível no momento em que a previsão precisaria ser feita na prática. Correlação confirmada de 0.564 com a target. **Excluída do modelo.**

> ⚠️ **Disponibilidade temporal de `repeat_purchase_30d`:** Essa variável indica se o cliente recomprou em até 30 dias após o pedido — ou seja, só é conhecida 30 dias após a entrega. Como o modelo precisa prever o NPS imediatamente após a entrega, ela não estaria disponível no momento ideal de aplicação em produção. Foram desenvolvidos e comparados dois modelos: um com e outro sem essa variável. Ver Seção 7.

> ⚠️ **Viés de resposta:** Clientes que respondem à pesquisa de NPS tendem a ser os mais satisfeitos ou os mais insatisfeitos — os neutros frequentemente não respondem. Isso pode criar um viés na base de dados que o modelo aprende e reproduz.

> ⚠️ **Uso como métrica absoluta:** O NPS não deve ser usado isoladamente para tomar decisões. Uma nota 7 pode ser satisfatória em um setor e insatisfatória em outro. O contexto sempre importa.
...
### Tipo de problema
Classificação multiclasse — prever a categoria NPS (Detrator / Neutro / Promotor) antes da aplicação da pesquisa, usando exclusivamente dados operacionais disponíveis durante a jornada de compra.

### Variáveis utilizadas
15 features operacionais após exclusão de identificadores e variáveis com data leakage.

### Separação dos dados
70% treino (1.750 registros) / 30% teste (750 registros) com estratificação por classe para manter a proporção do desbalanceamento.

### Modelos testados e resultados

| Modelo | F1 Weighted | F1 Detrator | F1 Neutro | F1 Promotor |
|---|---|---|---|---|
| Baseline (DummyClassifier) | 0.6294 | 0.85 | 0.00 | 0.00 |
| Random Forest | 0.8130 | 0.91 | 0.35 | 0.97 |
| XGBoost (com repeat_purchase) | 0.8249 | 0.89 | 0.49 | 0.95 |
| XGBoost (sem repeat_purchase) | 0.7412 | 0.88 | 0.40 | 0.26 |

### Modelo escolhido para produção
**XGBoost sem `repeat_purchase_30d`**

**Justificativa de negócio:**
- Disponível imediatamente após a entrega — sem necessidade de aguardar 30 dias
- F1 Detrator = 0.88 — detecta 88% dos clientes insatisfeitos
- Alinhado ao objetivo principal: identificar e agir sobre detratores de forma proativa
- Ainda 18% superior ao baseline

**Justificativa técnica:**
- `repeat_purchase_30d` não está disponível no momento em que o modelo precisa ser aplicado na vida real
- A remoção causou queda de 0.08 no F1 geral — aceitável dado o ganho em aplicabilidade
- F1 Detrator praticamente mantido (0.89 → 0.88) — classe prioritária para o negócio

### Desbalanceamento de classes
A base apresenta 74% de Detratores, 18% de Neutros e 8% de Promotores. Tratado com `class_weight='balanced'` no Random Forest e `sample_weight` no XGBoost.

### Interpretabilidade
Análise SHAP aplicada ao modelo final revelou que:
- Para **Detratores**: principais drivers são complaints_count e delivery_delay_days
- Para **Promotores**: repeat_purchase_30d domina a previsão (quando disponível)
- Confirma consistência com os achados da análise exploratória
...
Após cada entrega
      ↓
Coletar dados operacionais (atraso, reclamações, contatos CS...)
      ↓
Aplicar modelo XGBoost (sem repeat_purchase)
      ↓
Cliente previsto como Detrator?
      ↓ SIM
CS aciona proativamente antes da pesquisa de NPS
      ↓
Monitorar conversão: Detrator → Neutro ou Promotor
      ↓
